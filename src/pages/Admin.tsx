...
import { Plus, Edit2, Trash2, CheckCircle2, History, Package, ClipboardList, User as UserIcon, Calendar, ArrowRight, DollarSign, Car, Search, ChevronDown, ChevronRight, TrendingUp, AlertTriangle, BarChart3, Users, LayoutDashboard, Zap } from 'lucide-react';
...
export default function Admin() {
  const { products, orders, vehicles, user, addProduct, updateProduct, deleteProduct, deleteOrder, toggleOrderStatus, addVehicle, updateVehicle, deleteVehicle, t, getLocalized } = useStore();
  const [activeTab, setActiveTab] = useState<'dashboard' | 'products' | 'orders' | 'vehicles'>('dashboard');
  const [showProductModal, setShowProductModal] = useState(false);
  const [showVehicleModal, setShowVehicleModal] = useState(false);
  const [editingProduct, setEditingProduct] = useState<Product | null>(null);
  const [editingVehicle, setEditingVehicle] = useState<Vehicle | null>(null);
  const [showInlineVehicleAdd, setShowInlineVehicleAdd] = useState(false);
  const [productForm, setProductForm] = useState<Partial<Product>>({ instock: true, rating: 4.5, compatibleVehicles: [] });
  ...
  const handleProductSubmit = (e: React.FormEvent) => {
    ...
    setProductForm({ instock: true, rating: 4.5, compatibleVehicles: [] });
  };
  ...
  const revenueByDay = orders.reduce((acc, order) => {
    const date = new Date(order.created).toLocaleDateString('en-GB', { day: '2-digit', month: 'short' });
    const existing = acc.find(d => d.date === date);
    if (existing) {
      existing.revenue += (order.totalAfterDiscount || order.subtotal);
    } else {
      acc.push({ date, revenue: (order.totalAfterDiscount || order.subtotal) });
    }
    return acc;
              ...
              <div className="lg:col-span-2 bg-white border border-zinc-200 rounded-[40px] p-8 lg:p-10">
                <div className="flex items-center justify-between mb-8">
                  ...
                  <div className="flex bg-zinc-50 p-1 rounded-xl">
                    <button className="px-4 py-1.5 rounded-lg text-[10px] font-bold bg-white text-zinc-900 shadow-sm uppercase tracking-widest border border-zinc-100">Live</button>
                    <button className="px-4 py-1.5 rounded-lg text-[10px] font-bold text-zinc-400 uppercase tracking-widest">History</button>
                  </div>
                </div>
          ...
          >
            <div className="flex justify-between items-center bg-white border border-zinc-200 rounded-3xl p-6 mb-8">
              ...
              <div className="flex gap-4">
                <button 
                  onClick={() => { setEditingProduct(null); setProductForm({ instock: true, rating: 4.5, compatibleVehicles: [] }); setShowProductModal(true); }}
                  ...
                  vehicleMakes.map(make => (
                    <div key={make} className="flex flex-col">
                      ...
                      >
                        ...
                        <span className="text-[10px] font-bold text-zinc-400 uppercase tracking-wider">{groupedVehicles[make].length} models</span>
                        ...
                        {expandedMakes.has(make) && (
                          ...
                          >
                            <div className="divide-y divide-zinc-50">
                              {groupedVehicles[make].map(v => (
                                <div key={v.id} className="grid grid-cols-[1fr_200px_120px] px-8 py-3.5 items-center hover:bg-sky-50/50 transition-colors group">
                                  <div className="flex flex-col">
                                    <span className="text-sm font-black text-zinc-900 uppercase tracking-tight">{v.model}</span>
                                    <span className="text-[10px] font-bold text-zinc-400 uppercase">{v.engine}</span>
                                  </div>
                                  <div className="text-xs font-black text-zinc-600 text-center bg-zinc-100/50 py-1.5 rounded-lg border border-zinc-100 inline-block px-3 mx-auto">
                                    {v.year}
                                  </div>
                                  <div className="flex justify-end gap-2 opacity-0 group-hover:opacity-100 transition-all transform translate-x-2 group-hover:translate-x-0">
                                    <button onClick={() => openVehicleEdit(v)} className="w-9 h-9 rounded-xl border border-zinc-200 flex items-center justify-center text-zinc-400 hover:text-sky-500 hover:bg-white hover:border-sky-100 transition-all shadow-sm">
                                      <Edit2 className="w-4 h-4" />
                                    </button>
                                    <button onClick={() => deleteVehicle(v.id)} className="w-9 h-9 rounded-xl border border-zinc-200 flex items-center justify-center text-zinc-400 hover:text-rose-500 hover:bg-white hover:border-rose-100 transition-all shadow-sm">
                                      <Trash2 className="w-4 h-4" />
                                    </button>
                                  </div>
                                </div>
        ...
        {showProductModal && (
          <div className="fixed inset-0 z-[100] flex items-center justify-center px-4">
            ...
            <motion.div 
              ...
              className="relative w-full max-w-2xl bg-white rounded-[40px] shadow-3xl p-8 lg:p-12"
            >
        ...
        {showVehicleModal && (
          <div className="fixed inset-0 z-[110] flex items-center justify-center px-4">
            ...
            <motion.div 
              ...
              animate={{ opacity: 1, scale: 1, y: 0 }}
              exit={{ opacity: 0, scale: 0.95, y: 20 }}
              className="relative w-full max-w-lg bg-white rounded-[40px] shadow-3xl p-8 lg:p-12"
            >
